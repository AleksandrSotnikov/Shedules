...
class MainViewModel() : ViewModel(), ContainerHost<DetailState, SideEffect> {
    ...
    fun updateHtml() = intent {
        val url =
            "https://docs.google.com/spreadsheets/d/e/2PACX-1vRXNw52ze21dKFbM7qlO9FVYR75syrbFAl296tRqAAia-EI_bfLZvYVQdACQLza9Udk-Uw_prCpCBok/pubhtml"
        ...
        if (state.html != null) {
            ...
            for (i in 0 until rows.size) {
                ...
                val cols: Elements =
                    row.select("td") // разбиваем полученную строку по тегу  на столбы
                ...
                if (cols.size == 10 && cols[0].toString() != "Группа" && cols[0].toString()
                    ...
                    paraList.list = paraList.list.plus(
                        ParaRaspes(
                            ...
                            cols[8].text(),
                            cols[9].text(),
                            date.substring(date.indexOf("Неделя") + 7)[0].toString(),
                            dayConvert(date.substring(date.indexOf("(") + 1, date.indexOf(")"))),
                            date
                        )
                    )
                }
    ...
    fun updateMainSchedules(mainSchedules: MainSchedules?) = intent {
        reduce {
            state.copy(mainSchedules = mainSchedules)
        }
    }

    fun updateSortedParaList(sortedParaRaspesList: List<ParaRaspes>) = intent {
        reduce {
            state.copy(sortedParaRaspesList = sortedParaRaspesList)
        }
    }
    ...
    fun updateEdited(groups: List<ParaRaspes>, main: List<Results>) = intent {
        reduce {
